...
import { TrendingUp, Home, Users, FileText, Clock, DollarSign, Shield, Settings, CheckCircle, ArrowRight, Star, ChevronDown, ChevronUp, Target, Award, BarChart3, Zap, Search, Hammer } from "lucide-react";
...
const Services = () => {
  ...
  const services = [{
    ...
    features: ['Transparent ROI tracking', 'Professional property selection', 'Complete renovation management', 'Shared profit distribution', 'Regular progress updates', 'Legal documentation included'],
    ...
    features: ['Investment advisory & strategy', 'Property transformation consulting', 'Premium property sourcing', 'End-to-end project management', 'HNWI Real Estate Concierge', 'Dubai relocation services (A-Z)'],
  ...
  const process = [{
    ...
    description: 'Website, WhatsApp, referrals'
  }, {
    step: 2,
    title: 'Client Qualification',
    description: 'Investment goals, budget, preferences'
  }, {
    step: 3,
    title: 'KYC & Due Diligence',
    description: 'Passport, Emirates ID, AML compliance'
  }, {
    step: 4,
    title: 'Project Presentation',
    description: 'Property analysis, financial projections'
  }, {
    step: 5,
    title: 'Legal Agreement Signing',
    description: 'NDA, JVA, investment agreements'
  }, {
    step: 6,
    title: 'Property Acquisition',
    description: 'Purchase, DLD registration, title transfer'
  }, {
    step: 7,
    title: 'Renovation Initiation',
    description: 'Timeline, status reports, site visits'
  }, {
    step: 8,
    title: 'Project Completion',
    description: 'Final inspection, handover, certification'
  }, {
    step: 9,
    title: 'Sale or Rental',
    description: 'Marketing, listing, buyer negotiations'
  }, {
    step: 10,
    title: 'Profit Distribution & Next Opportunity',
    description: 'ROI payout, reinvestment options, new deals'
  }];
  return <>
      <Navigation />
      ...
      <section className="relative h-96 flex items-center">
        ...
        backgroundImage: `url(${renovationShowcase})`
      }}>
      ...
      <div className="container mx-auto px-4 py-12 space-y-12">
        ...
        <div className="grid grid-cols-1 gap-8">
          ...
          return <Card key={service.id} className={`relative hover:shadow-md transition-shadow ${service.highlight ? 'ring-2 ring-primary' : ''}`}>
                {service.highlight && <div className="absolute -top-3 left-6">
                ...
                <div className="flex flex-col lg:flex-row">
                  <div className="flex-1">
                    <CardHeader className="pb-4">
                      <div className="flex items-start justify-between">
                        ...
                        <Button variant="outline" size="sm" onClick={() => setExpandedService(isExpanded ? null : service.id)} className="ml-4 bg-primary/10 border-primary/30 hover:bg-primary/20 text-primary font-medium">
                    ...
                    <CardContent className="space-y-4">
                      <div className="space-y-2">
                        {service.features.slice(0, 4).map((feature, index) => <div key={index} className="flex items-center gap-2">
                            <CheckCircle className="h-4 w-4 text-success flex-shrink-0" />
                            <span className="text-sm">{feature}</span>
                          </div>)}
                        {service.features.length > 4 && <p className="text-xs text-muted-foreground">+{service.features.length - 4} more features</p>}
                      </div>

                      {/* Expanded Options */}
                      {isExpanded && service.expandedOptions && <div className="mt-6 pt-6 border-t">
                          <h4 className="text-lg font-semibold mb-4">Available Options:</h4>
                          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            {service.expandedOptions.map((option, index) => <Card key={index} className={`p-4 ${option.popular ? 'ring-2 ring-primary' : 'border'}`}>
                                {option.popular && <div className="absolute -top-2 left-1/2 transform -translate-x-1/2">
                                    <Badge className="bg-primary text-primary-foreground text-xs">Most Popular</Badge>
                                  </div>}
                                <div className="text-center mb-4">
                                  <h5 className="font-semibold text-lg mb-1">{option.title}</h5>
                                  {option.returns && <div className="text-2xl font-bold text-primary mb-1">{option.returns}</div>}
                                  {option.investment && <p className="text-sm text-muted-foreground mb-2">Min. {option.investment}</p>}
                                  <p className="text-sm text-muted-foreground">{option.description}</p>
                                </div>
                                <ul className="space-y-2 mb-4">
                                  {option.features.map((feature, featureIndex) => <li key={featureIndex} className="flex items-start gap-2">
                                      <CheckCircle className="h-4 w-4 text-success flex-shrink-0 mt-0.5" />
                                      <span className="text-sm">{feature}</span>
                                    </li>)}
                                </ul>
                                <Button variant={option.popular ? "default" : "outline"} size="sm" className="w-full" onClick={() => window.location.href = '/contact'}>
                                  Learn More
                                </Button>
                              </Card>)}
                          </div>
                  ...
                  <div className="w-full lg:w-80 lg:flex lg:flex-col lg:justify-between p-6">
                    <div className="aspect-video lg:aspect-[4/3] overflow-hidden rounded-lg mb-4 group">
                      <img src={serviceImage} alt={service.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" />
                    </div>
                    <Button variant={service.highlight ? "luxury" : "outline"} className="w-full" onClick={() => window.location.href = '/contact'}>
                      {service.cta}
          ...
          <CardContent className="p-0">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 p-8">
              <div className="relative overflow-hidden rounded-2xl group hover:scale-105 transition-all duration-500">
                ...
                <img src="/src/assets/premium-selection.jpg" alt="Property selection and analysis" className="w-full h-80 object-cover group-hover:scale-110 transition-transform duration-700" />
              ...
              <div className="relative overflow-hidden rounded-2xl group hover:scale-105 transition-all duration-500">
                ...
                <img src="/src/assets/luxury-transformation.jpg" alt="Luxury renovation showcase" className="w-full h-80 object-cover group-hover:scale-110 transition-transform duration-700" />
                <div className="absolute inset-0 z-20 p-8 flex flex-col justify-between text-white">
                  <div className="w-16 h-16 bg-white/20 backdrop-blur-sm rounded-full flex items-center justify-center">
              ...
              <div className="relative overflow-hidden rounded-2xl group hover:scale-105 transition-all duration-500">
                ...
                <img src="/src/assets/exceptional-returns.jpg" alt="Profitable sale and returns" className="w-full h-80 object-cover group-hover:scale-110 transition-transform duration-700" />
                <div className="absolute inset-0 z-20 p-8 flex flex-col justify-between text-white">
                  <div className="w-16 h-16 bg-white/20 backdrop-blur-sm rounded-full flex items-center justify-center">
          ...
          <CardContent>
            ...
            <div className="mb-8 p-6 bg-primary/5 rounded-lg border border-primary/20">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {process.slice(0, 3).map(step => <div key={step.step} className="relative">
                    ...
                    {step.step < 3 && <div className="hidden md:block absolute top-1/2 -right-2 transform -translate-y-1/2 z-10">
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
                {process.slice(3).map(step => <div key={step.step} className="relative">
                    ...
                    {step.step < 10 && <div className="hidden lg:block absolute top-1/2 -right-2 transform -translate-y-1/2">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              <div className="border rounded-lg overflow-hidden hover:shadow-md transition-shadow">
                <div className="aspect-video overflow-hidden">
                  <img src={dubaeMarinaLuxury} alt="Dubai Marina" className="w-full h-full object-cover hover:scale-105 transition-transform duration-300" />
                </div>
                <div className="p-6">
              ...
              <div className="border rounded-lg overflow-hidden hover:shadow-md transition-shadow">
                <div className="aspect-video overflow-hidden">
                  <img src={downtownLuxury} alt="Downtown Dubai" className="w-full h-full object-cover hover:scale-105 transition-transform duration-300" />
                </div>
                <div className="p-6">
              ...
              <div className="border rounded-lg overflow-hidden hover:shadow-md transition-shadow">
                <div className="aspect-video overflow-hidden">
                  <img src={businessBay} alt="Business Bay" className="w-full h-full object-cover hover:scale-105 transition-transform duration-300" />
                </div>
                <div className="p-6">
              ...
              <div className="border rounded-lg overflow-hidden hover:shadow-md transition-shadow">
                <div className="aspect-video overflow-hidden">
                  <img src={marinaTower} alt="Palm Jumeirah" className="w-full h-full object-cover hover:scale-105 transition-transform duration-300" />
                </div>
                <div className="p-6">
              ...
              <div className="border rounded-lg overflow-hidden hover:shadow-md transition-shadow">
                <div className="aspect-video overflow-hidden">
                  <img src={premiumVillaSelection} alt="Dubai Hills Estate" className="w-full h-full object-cover hover:scale-105 transition-transform duration-300" />
                </div>
                <div className="p-6">
              ...
              <div className="border rounded-lg overflow-hidden hover:shadow-md transition-shadow">
                <div className="aspect-video overflow-hidden">
                  <img src={premiumTower} alt="JBR - Jumeirah Beach Residence" className="w-full h-full object-cover hover:scale-105 transition-transform duration-300" />
                </div>
                <div className="p-6">
        ...
        <Card className="bg-gradient-luxury text-background">
          <CardContent className="p-8 text-center">
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <Button variant="outline" size="lg" className="bg-background text-foreground hover:bg-background/90" onClick={() => window.location.href = '/contact'}>
