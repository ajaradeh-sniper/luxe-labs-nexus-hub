...
-- Sample projects (only add if not existing)
INSERT INTO public.projects (name, description, project_type, status, budget, start_date, end_date, created_by) 
SELECT 'Downtown Luxury Tower', 'Premium residential development in the heart of downtown', 'development', 'active', 15000000, '2024-01-15', '2025-12-31', (SELECT user_id FROM profiles WHERE role = 'administrator' LIMIT 1)
WHERE NOT EXISTS (SELECT 1 FROM projects WHERE name = 'Downtown Luxury Tower');

INSERT INTO public.projects (name, description, project_type, status, budget, start_date, end_date, created_by) 
SELECT 'Marina Bay Renovation', 'Complete renovation of existing commercial complex', 'renovation', 'planning', 8500000, '2024-03-01', '2024-11-30', (SELECT user_id FROM profiles WHERE role = 'administrator' LIMIT 1)
WHERE NOT EXISTS (SELECT 1 FROM projects WHERE name = 'Marina Bay Renovation');

INSERT INTO public.projects (name, description, project_type, status, budget, start_date, end_date, created_by) 
SELECT 'Green Valley Villas', 'Sustainable housing development project', 'development', 'active', 12000000, '2024-02-01', '2025-08-15', (SELECT user_id FROM profiles WHERE role = 'administrator' LIMIT 1)
WHERE NOT EXISTS (SELECT 1 FROM projects WHERE name = 'Green Valley Villas');
...
AND NOT EXISTS (SELECT 1 FROM project_costs WHERE description = 'Foundation and structural work');

-- Sample project risks
INSERT INTO public.project_risks (project_id, title, description, risk_level, category, probability, impact_score, status, mitigation_plan, created_by) 
SELECT 
    (SELECT id FROM projects WHERE name = 'Downtown Luxury Tower' LIMIT 1), 
    'Weather Delays', 
    'Potential construction delays due to severe weather conditions', 
    'medium', 
    'environmental', 
    0.3, 
    6, 
    'monitored', 
    'Implement flexible scheduling and weather-resistant construction methods', 
    (SELECT user_id FROM profiles WHERE role = 'administrator' LIMIT 1)
WHERE EXISTS (SELECT 1 FROM projects WHERE name = 'Downtown Luxury Tower')
AND NOT EXISTS (SELECT 1 FROM project_risks WHERE title = 'Weather Delays');
...
SELECT 'active_projects', 'count', 'projects', 3, '{"status": "active", "total_value": 35500000}'
